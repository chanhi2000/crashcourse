...
lang: ko-KR
title: 1. 리눅스 기초
description: Bash 쉘스크립트 개발 시작하기 > 1. 리눅스 기초
category: 
  - CLI
  - Linux
tag: 
...
head:
  - - meta:
    - property: og:title
      content: Bash 쉘스크립트 개발 시작하기 > 1. 리눅스 기초
...
`yum`은 레드햇 계열, `apt-get`은 데비안 계열에서 사용하는 패키지 설치 명령입니다. 이 명령은 패키지간의 의존성을 확인해서 자동으로 설치해 주기 때문에 사용이 편리합니다.
...
`locale` 명령으로 현재 언어 설정을 확인합니다. LANG 가 `eu_US`로 되어 있으면 영어 설정입니다. 이 부분을 `en_US..UTF-8`로 수정하면 됩니다. <FontIcon icon="iconfont icon-file"/>`/etc/default/locale` 파일을 `vi` 로 수정하면 됩니다.
...
- 기본 설정 변경
  - .<FontIcon icon="iconfont icon-file"/>`~/.vimrc` 파일에 `set encoding=utf-8`을 입력
  - .<FontIcon icon="iconfont icon-file"/>`/etc/vim/vimrc` 파일에 `set encoding=utf-8`을 입력
