...
크론탭을 일괄로 등록하고 싶을 때 <FontIcon icon="fas fa-terminal"/>`echo` 명령을 이용하여 처리할 수 있습니다. <FontIcon icon="fas fa-folder-open"/>`/var/spool/cron/<유저명>`에 유저별 크론탭이 있습니다. 여기에 넣어주면 <FontIcon icon="fas fa-terminal"/>`crontab -e`와 동일한 효과를 얻을 수 있습니다.
...
주어진 명령어를 실행하는데 새로운 프로세스를 생성하지 않고, 쉘 프로세스를 대체합니다. 예를 들어 `bash` 쉘에서 자바 프로그램을 실행하면 자바 프로그램의 ppid가 `bash` 쉘이 되고, 자바 프로그램이 `bash` 쉘의 하위 프로세스로 실행됩니다. <FontIcon icon="fas fa-terminal"/>`exec` 커맨드로 실행하면 `bash`쉘의 프로세스가 자바 프로그램이 됩니다. ppid가 따로 업습니다. 그리고 자바프로그램이 종료되면 프로세스가 종료됩니다. `bash` 쉘로 돌아오지 않습니다.
