...
일일이 <FontIcon icon="fas fa-terminal"/>`ssh -i awskeypair.pem`처럼 <FontIcon icon="iconfont icon-token"/>`pem` 파일을 지정하기 귀찮다면 <FontIcon icon="fas fa-file-lines"/>`/etc/ssh/ssh_config` 파일을 열고 맨 아래 다음과 같이 추가해줍니다(Mac OS X에서는 <FontIcon icon="fas fa-file-lines"/>`/etc/ssh_config`).
