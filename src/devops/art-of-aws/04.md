...
구글에서 putty로 검색하거나 [http://www.chiark.greenend.org.uk/~sgtatham/putty](http://www.chiark.greenend.org.uk/~sgtatham/putty) 주소에 접속합니다. <FontIcon icon="iconfont icon-select"/>`[Download]` 링크를 클릭하고, 이 Download 페이지에서 <FontIcon icon="iconfont icon-file"/>`putty.zip`을 찾은 뒤 파일을 다운로드합니다.

::: tabs

@tab:active 1

![.<FontIcon icon="iconfont icon-file"/>`putty.zip` 파일의 압축을 풀면 다음과 같은 파일들이 나옵니다 (<FontIcon icon="iconfont icon-play"/>`putty-0.XX-installer.exe` 설치파일을 다운로드하고 설치해도 상관없습니다).](https://pyrasis.com/assets/images/TheArtOfAmazonWebServicesChapter04/17.png)
