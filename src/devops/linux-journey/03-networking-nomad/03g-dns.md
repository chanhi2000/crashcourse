...
The <FontIcon icon="fas fa-file-lines"/>`/etc/hosts` file contains mappings of some hostnames to IP addresses. The fields are pretty self explanatory, there is one for the IP address, the hostname and then any alias's for the host.
...
### <FontIcon icon="fas fa-file-lines"/>`/etc/resolv.conf`

Traditionally, we've used a file called <FontIcon icon="fas fa-file-lines"/>`/etc/resolv.conf` to map DNS name servers for more efficient lookups, however with the improvements made to DNS this file is quite often irrelevant, in fact, you can see in my example below that <FontIcon icon="fas fa-file-lines"/>`/etc/resolv.conf` isn't managed manually. Refer to your distribution specific settings to manage DNS name server mappings.
