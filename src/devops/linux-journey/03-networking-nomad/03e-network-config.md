...
The status of interfaces, can be up or down, as you can guess if you wanted to "turn off" an interface you can set it to go down. The fields you'll probably look at the most in the `ifconfig` output is the `HWaddr` (_MAC address of the interface_), `inet address` (IPv4 address) and `inet6` (IPv6 address). Of course you can see that the subnet mask and broadcast address are there as well. You can also view interface information at <FontIcon icon="fas fa-folder-open"/>`/etc/network/interfaces`.
