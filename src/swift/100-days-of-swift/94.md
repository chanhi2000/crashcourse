...
lang: ko-KR
title: Day 94
description: 100 Days of Swift > Day 94
category:
  - Swift
tag: 
  - crashcourse
...
head:
  - - meta:
    - property: og:title
      content: 100 Days of Swift > Day 94
...
We've been mixing UIKit and SpriteKit ever since our first SpriteKit project, way back in project 11. Don't believe me? Look inside <FontIcon icon="fa-brands fa-swift"/>`GameViewController.swift` and you'll see a plain old `UIViewController` do all the work of loading and showing our `GameScene` code. There's a Main.storyboard file containing that view controller, and if you go to the identity inspector (<kbd>Alt</kbd>+<kbd>Cmd</kbd>+<kbd>3</kbd>) you'll see it has `SKView` set for its custom class – that's the view holding our scene.
