...
lang: ko-KR
title: Day 42
description: 100 Days of Swift > Day 42
category:
  - Swift
tag: 
  - crashcourse
...
Now open <FontIcon icon="fas fa-file-lines"/>`Main.storyboard` in Interface Builder and delete the existing view controller. In its place, drag out a Collection View Controller (_not_ a regular collection view!), then mark it as the initial view controller and embed it inside a navigation controller. Make sure you also use the identity inspector to change its class to “ViewController” so that it points to our class in code.
