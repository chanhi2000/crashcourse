...
lang: ko-KR
title: Day 68
description: 🕊️100 Days of Swift > Day 68
tags: ["crashcourse", "swift" , "paul-hudson" , "xcode", "appstore"]
meta:
  - name: 100 Days of Swift > Day 68
    content: Day 68
  - property: og:title
    content: Day 68
...
First, we're going to modify <FontIcon icon="iconfont icon-nodeJS"/>`Action.js` to send two pieces of data to our extension: the URL the user was visiting, and the title of the page. Go to <FontIcon icon="iconfont icon-nodeJS"/>`Action.js` and modify the `run()` function to this:
...
![Xcode will ask you which app you should run with your extension. Please choose Safari.](https://www.hackingwithswift.com/img/books/hws/19-5.png)
...
In a Safari extension like ours, the data we return here will be passed in to the `finalize()` function in the <FontIcon icon="iconfont icon-nodeJS"/>`Action.js` JavaScript file, so we're going to modify the `done()` method so that it passes back the text the user entered into our text view.
...
That's all the code required to send data back to Safari, at which point it will appear inside the `finalize()` function in <FontIcon icon="iconfont icon-nodeJS"/>`Action.js`. From there we can do what we like with it, but in this project the JavaScript we need to write is remarkably simple: we pull the "customJavaScript" value out of the `parameters` array, then pass it to the JavaScript `eval()` function, which executes any code it finds.

Open <FontIcon icon="iconfont icon-nodeJS"/>`Action.js`, and change the `finalize()` function to this:
...
__Trivia__: My first Swift app, written back when Swift was first launched in iOS 8.0, was a Safari action. They had just been introduced, and I wanted to solve a problem I had: I wanted to be able to use View Source in Safari.