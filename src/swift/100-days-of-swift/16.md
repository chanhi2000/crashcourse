...
Up to this point we’ve been working entirely in the file <FontIcon icon="fa-brands fa-swift"/>`ViewController.swift`, but now I’d like you to use the project navigator (the pane on the left) to select the file <FontIcon icon="fas fa-file-lines"/>`Main.storyboard`. Storyboards contain the user interface for your app, and let you visualize some or all of it on a single screen.

When you select <FontIcon icon="fas fa-file-lines"/>`Main.storyboard`, you’ll switch to the Interface Builder visual editor, and you should see something like the picture below:
