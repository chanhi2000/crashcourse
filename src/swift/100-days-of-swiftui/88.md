...
lang: ko-KR
title: Day 88
description: 100 Days of SwiftUI > Day 88
category:
  - Swift
tag: 
  - crashcourse
...
head:
  - - meta:
    - property: og:title
      content: 100 Days of SwiftUI > Day 88
...
So, create a new Swift file called <FontIcon icon="fa-brands fa-swift"/>`Card.swift` and give it this code:
...
That’s our card view done for the time being, so if you want to see it in action go back to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift` and replace its `body` property with this:
...
Add this extension to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift`, outside of the `ContentView` struct:
