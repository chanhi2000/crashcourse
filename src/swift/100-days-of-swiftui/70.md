...
lang: ko-KR
title: Day 70
description: 100 Days of SwiftUI > Day 70
category:
  - Swift
tag: 
  - crashcourse
...
So, create a new Swift file called <FontIcon icon="fa-brands fa-swift"/>`Location.swift`, giving it this code:
...
First, I don’t particularly like having to make a `CLLocationCoordinate2D` inside our SwiftUI view, and I’d much prefer to move that kind of logic inside our `Location` struct. So, we can move that into a computed property to clean up our code. First, add an import for MapKit into <FontIcon icon="fa-brands fa-swift"/>`Location.swift`, then add this to `Location`:
