...
lang: ko-KR
title: Day 39
description: 100 Days of SwiftUI > Day 39
category:
  - Swift
tag: 
  - crashcourse
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - Moonshot: Introduction",
  "desc": "Moonshot: Introduction",
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - Resizing images to fit the screen using GeometryReader",
...
<VidStack src="youtube/Zzx9JgMNzkM" />
...
__Tip__: When you're using fixed image names such as this one, Xcode generates constant names for them all that you can use in place of strings. In this case, that means writing `Image(.example)`, which is much safer than using a string!
...
__Important__: This stops your preview from running live, so you won't be able to interact with your view until you select the Live option instead.
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - How ScrollView lets us work with scrolling data",
  "desc": "How ScrollView lets us work with scrolling data",
...
<VidStack src="youtube/GWivGqus6hE" />

You’ve seen how `List` and `Form` let us create scrolling tables of data, but for times when we want to scroll _arbitrary_ data – i.e., just some views we’ve created by hand – we need to turn to SwiftUI’s `ScrollView`.
...
If you run that back in the simulator you’ll see that you can drag the scroll view around freely, and if you scroll to the bottom you’ll also see that `ScrollView` treats the safe area just like `List` and `Form` – their content goes _under_ the home indicator, but they add some extra padding so the final views are fully visible.

You might also notice that it’s a bit annoying having to tap directly in the center – it’s more common to have the whole area scrollable. To get _that_ behavior, we should make the `VStack` take up more space while leaving the default centre alignment intact, like this:
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - Pushing new views onto the stack using NavigationLink",
  "desc": "Pushing new views onto the stack using NavigationLink",
...
<VidStack src="youtube/qhZPTZ52kpo" />
...
This view stack system is very different from the sheets we’ve used previously. Yes, both show some sort of new view, but there’s a difference in the way they are presented that affects the _way_ users think about them.
...
So, both `sheet()` and `NavigationLink` allow us to show a new view from the current one, but the _way_ they do it is different and you should choose them carefully:
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - Working with hierarchical Codable data",
  "desc": "Working with hierarchical Codable data",
...
<VidStack src="youtube/1V1uMYB-Rzg" />
...
```component VPCard
{
  "title": "100 Days of SwiftUI - Day 39 - How to lay out views in a scrolling grid",
  "desc": "How to lay out views in a scrolling grid",
...
<VidStack src="youtube/G1Fu_275DwQ" />

SwiftUI’s `List` view is a great way to show scrolling rows of data, but sometimes you also want _columns_ of data – a grid of information, that is able to adapt to show more data on larger screens.
...
That works for some situations, but the best part of grids is their ability to work across a variety of screen sizes. This can be done with a different column layout using _adaptive_ sizes, like this:
...
Before we’re done, I want to briefly show you how to make _horizontal_ grids. The process is almost identical, you just need to make your `ScrollView` work horizontally, then create a `LazyHGrid` using rows rather than columns:
...
That brings us to the end of the overview for this project, so please go ahead and reset <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift` to its original state.
