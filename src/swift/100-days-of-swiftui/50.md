...
So, please make a new Swift file called <FontIcon icon="fa-brands fa-swift"/>`Order.swift`, change its Foundation import for SwiftUI, and give it this code:
...
We’ll make that more useful shortly, but for now it means we can return to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift` and add the final section for our form. This will create a `NavigationLink` that points to an `AddressView`, passing in the current order object.
...
<VidStack src="youtube/vOwD9q5r2cQ" />
