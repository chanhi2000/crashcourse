...
lang: ko-KR
title: Day 82
description: 100 Days of SwiftUI > Day 82
category:
  - Swift
tag: 
  - crashcourse
...
head:
  - - meta:
    - property: og:title
      content: 100 Days of SwiftUI > Day 82
...
Now that we have something to store, we can tell SwiftData to create a model container for it. This means going to <FontIcon icon="fa-brands fa-swift"/>`HotProspectsApp.swift`, giving it an import for SwiftData, then adding the `modelContainer(for:)` modifier like this:
...
So, go ahead and open <FontIcon icon="fa-brands fa-swift"/>`ProspectsView.swift`, give it an import for SwiftData, then add these two new properties to the `ProspectsView` struct:
