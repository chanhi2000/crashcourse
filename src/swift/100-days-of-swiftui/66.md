...
First, we need to add two new properties: one to get the reviewer requester from SwiftUI's environment, and one to track how many filter changes have happened. Add another import to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift`, this time for StoreKit, then add these two properties to ContentView`:
