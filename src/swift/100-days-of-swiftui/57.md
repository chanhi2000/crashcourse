...
lang: ko-KR
title: Day 57
description: 100 Days of SwiftUI > Day 57
category:
  - Swift
tag: 
  - crashcourse
...
To demonstrate this, we could create a simple `User` class with a handful of properties. Create a new file called <FontIcon icon="fa-brands fa-swift"/>`User.swift`, add an import at the top for SwiftData, then give it this code:
...
Let's build this step by step. First, open <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift` and an import for SwiftData, then add properties to get access to the model context, load all our `User` objects, then store a path we can bind to a `NavigationStack`:
