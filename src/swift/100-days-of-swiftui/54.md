...
First, the model: create a new file called <FontIcon icon="fa-brands fa-swift"/>`Book.swift`, add an import for SwiftData, then give it this code:
...
Now that we have our data model, we can ask SwiftData to create a model container for it. This means opening <FontIcon icon="fa-brands fa-swift"/>`BookwormApp.swift`, adding `import SwiftData` to the top of the file, then adding this modifier to the `WindowGroup`:
...
Showing `AddBookView` involves returning to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift` and following the usual steps for a sheet:
...
Enough talk – let’s start writing some more code. Please start by adding an import for SwiftData to <FontIcon icon="fa-brands fa-swift"/>`ContentView.swift`, then add these properties to the `ContentView` struct:
