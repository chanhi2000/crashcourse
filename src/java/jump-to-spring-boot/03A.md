...
메뉴 버튼을 클릭해야 숨겨진 "로그인" 링크가 보일 것이다. 하지만 아직은 메뉴 버튼을 클릭해도 아무런 변화가 없다. 그 이유는 부트스트랩 자바스크립트 파일(<FontIcon icon="fa-brands fa-js"/>`bootstrap.min.js`)이 <FontIcon icon="iconfont icon-page"/>`layout.html` 파일에 포함되지 않았기 때문이다.
