...
head:
  - - meta:
    - property: og:title
      content: Jump to Spring Boot > 3-02. 페이징
...
> 파일명: <FontIcon icon="iconfont icon-folder"/>`/sbb/src/main/java/com/mysite/sbb/question/`<FontIcon icon="fa-brands fa-java"/>`QuestionRepository.java`
...
.<FontIcon icon="fa-brands fa-html"/>`question_list.html` 템플릿 파일의 `</table>` 태그 바로 밑에 다음 코드를 추가하자.
